...
type Props = {
    ...
    /** Opcjonalna flaga: jeśli true, ukrywa zespoły ze statusem ARCHIVED */
    hideArchived?: boolean
}

export default function TeamSearchModal({ open, onOpenChange, onSelect, hideArchived }: Props) {
    ...
    // Logika filtrowania: jeśli hideArchived jest włączone, odrzucamy elementy ze statusem ARCHIVED
    const filteredItems = useMemo(() => {
        if (!hideArchived) return clubs.items;
        return clubs.items.filter((c: any) => {
            const status = c.status || c.teamStatus || '';
            return status.toUpperCase() !== 'ARCHIVED';
        });
    }, [clubs.items, hideArchived]);
            ...
            <DialogContent>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        {!loading && filteredItems.map((c, idx) => {
                                ...
                                <div key={clubId ?? `${clubName}-${idx}`} className="rounded-lg border p-3 bg-card flex items-center justify-between hover:shadow">
                                    <div>
                                        <div className="flex items-center gap-3">
                                            ...
                                            {/* Opcjonalnie: pokaż status w debugu lub badge */}
